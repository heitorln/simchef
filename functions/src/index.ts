...
import {CieloConstructor, Cielo,
  TransactionCreditCardRequestModel, EnumBrands,
  CaptureRequestModel,
  CancelTransactionRequestModel} from "cielo";
    ...
    async (data, context) => {
      ...
      try {
        ...
        if (transaction.payment.status === 1) {
          ...
          return {
            "sucess": false,
            "stauts": transaction.payment.status,
            "error": {
...
export const cancelCreditCard = functions.https.onCall(
    async (data, context) => {
      ...
      try {
        const cancel = await cielo.creditCard.cancelTransaction(
            cancelParams
        );
...
export const helloWorld = functions.https.onCall((data, context) => {
  return {data: "Hello from Cloud Funtions!!!"};
});
...
export const onNewOrder = functions.firestore.document("/orders/{orderId}")
    .onCreate(async (snapshot, context) => {
      ...
      // const querySnapshot = await admin.firestore().collection('').get()
      const snap = await admin.firestore()
          .collection("orders").doc(orderId).get();

      const profissionalId = snap.get("profissionalId");

      const tokensProfissionalIds: string[] = await getDeviceTokens(
          profissionalId
      );

      console.log(orderId, profissionalId, tokensProfissionalIds);
      await sendPushFCM(
          tokensProfissionalIds,
          "Novo Pedido",
